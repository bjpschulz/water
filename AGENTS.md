...
- Resolution/horizon decisions must also be evidence-gated: hypothesis -> experiment -> decision
...
Relevant files, in pipeline order — each is generated from the one above it:

1. `data/Water_WHW.csv` — **raw data**: whole-house water, 1-minute measurements.
   Columns:
   - `unix_ts` — Unix timestamp (true UTC; see "Settled facts")
   - `counter` — cumulative volume, in Liters (L)
   - `avg_rate` — volume consumed during the sampling interval; **primary
     target** (for 1-minute data, effectively L/min)
   - `inst_rate` — instantaneous flow-rate estimate

2. `data/processed/whw_v100.parquet` (+ `whw_v100.csv` twin) — **cleaned
   analysis period**: the V100-meter data starting at the
   pulse-grid-consistent timestamp (see "Water-meter period" below), keyed
   by `unix_ts`. Columns: `datetime_local`, `counter`, `avg_rate`.
   `inst_rate` and a UTC datetime column are dropped (unused by the
   analyses; re-derivable from the raw CSV).
   Caution: never index or resample on `datetime_local` — it is non-unique
   across DST fall-back and skips the spring-forward hour.
   Generated by: `src/initial_eda.py`.

3. `data/processed/whw_features_v1.parquet` (+ `whw_features_v1.csv` twin) —
   **modelling feature table**; the file all downstream stages (splits,
   baselines, models) operate on. Same `unix_ts` grid; cleaned columns
   plus the lag and calendar feature groups. All features are
   deterministic row-wise transforms (backward shifts, timestamp
   functions), so the table is built on the full series before splitting
   without leakage; anything fitted to the target stays downstream of the
   split. Lag warm-up NaN rows are kept — the chronological split
   (`src/splits.py`) excludes them from train.
   The `v1` suffix is deliberate: the feature set is provisional until the
   Stage 5+ ablation settles it. If columns are added, dropped, or
   redefined, bump the version (`v2`) and regenerate — never edit a
   version in place.
   Generated by: `src/build_features.py`.

Water meters work through **pulses**: each pulse represents a fixed volume passing through the meter.
There was a meter replacement on **2012-07-14 (Unix timestamp 1342287780)**.
The old meters used roughly 1 gallon (3.785 L) per pulse; the replacement V100 meters use roughly 0.5 L per pulse.
...
Implemented in `src/splits.py`: downstream stages consume `split_series()`
in-memory; no split data artifacts are written.
...
- Only major scripts (pipeline stages producing measured results/artifacts)
  get thorough documentation and a `results/<stage>/summary.json`. Helper
