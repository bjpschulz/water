...
3. `data/processed/whw_features_v1.parquet` (+ `whw_features_v1.csv` twin) —
   **modelling feature table**; the file all downstream stages (splits,
   ...
   split. Lag warm-up NaN rows are kept — the chronological split must
   place them inside train.
   ...
   version in place.
   Generated by: `src/build_features.py`. Column inventory and
   self-checks: `results/build_features/summary.json`; feature rationale
   and per-model encoding rules: `FEATURES.md`.
