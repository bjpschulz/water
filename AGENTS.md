...
- `data/processed/whw_v100.parquet` (+ `whw_v100.csv` twin) — cleaned V100
  period, keyed by `unix_ts`, columns `datetime_local`, `counter`,
  `avg_rate`. `inst_rate` and a UTC datetime column are dropped from the
