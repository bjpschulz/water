...
- **V100 period only** for all analysis (`unix_ts >= 1342287780`); earlier
  ...
  swap (no reset). (`scripts/eda_whw.py` → `meter_transition` in
  `summary.json`.)
- **`unix_ts` is true Unix/UTC time**, not local time encoded as UTC.
  ...
  Dot reproducible from current scripts — treat as settled
  unless there's a specific reason to re-derive it.
